...
axios({
    method: 'get',
    url: 'http://localhost:8090/checkRequest/findfinishPatientCount',
    params: {
     state:'已检查'
    }
  })
...
function findPatient(){
  //清空数据
    ...
    axios({
        method: 'get',
        url: 'http://localhost:8090/checkRequest/findPatient',
        params: {
...
function createMecical(index,row){
    ...
    axios({
        method: 'post',
        url: 'http://localhost:8090/checkApply/patientCheckApply',
        data: {
            id:row.id,
            state:'已检查'
        } 
